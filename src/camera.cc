...
void Camera::Initialize() {
  ...
  auto viewport_upper_left =
      Camera::center_ - P - (viewport_u / 2) - (viewport_v / 2);
  pixel00_loc_ = viewport_upper_left +
                 0.5 * (Camera::pixel_delta_u_ + Camera::pixel_delta_v_);
}

Vec3 Camera::SampleSquare() const {
  // Returns a vector to a random point in the square [-0.5, 0.5) x [-0.5, 0.5)
  // x 0
...
void Camera::Render(const Hittable &world) {
  ...
  for (int j = 0; j < Camera::image_h_; j++) {
    std::clog << "\rScanlines remaining: " << (Camera::image_h_ - j) << ' '
              << std::flush;
